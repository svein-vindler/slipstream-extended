...
1. **Create a fine-grained token** for triggering:
   ...
   - Repository access → **Only select repositories** → your `slipstream-extended` repo
...
2. **Build the Shortcut** (Shortcuts app → **+**):
   ...
   - URL:
     ```
     https://api.github.com/repos/<you>/slipstream-extended/actions/workflows/refresh.yml/dispatches
     ```
   - Method: **POST**
   - Headers:
